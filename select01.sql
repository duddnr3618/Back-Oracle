...
from employee
where dno = 20 and hiredate >= '81/4/01';            --문자열 : '' / 숫자 : '' x 

--<문제3> 연봉을 계산해서 사원번호, 사원이름, 월급, 보너스, 전체 연봉을 출력
select eno 사원번호 , ename 사원이름 , salary 월급 , commission 보너스 ,
salary*12 +NVL ( commission , 0) 전체연봉 --null 값을 사칙연산 하게 되면 null값이 나온다. -> NVL 으로 null값을 0으로 바꿔준다.
from employee
...
select ename 사원이름 , hiredate 입사날짜 , dno 부서이름 , salary 월급
from employee
...
--where절에서 별칭이름을 사용 하면 오류 발생
-- order 절에서는 별칭이름 사용 가능
