...
select sysdate -1 as 어제 , sysdate 오늘 , sysdate+1 as 내일
from dual;

-- 입사일 부터 오늘날짜까지 몇일 근무 했는지 출력
select ename 이름 , hiredate 입사일 ,ROUND(sysdate - hiredate) 근무일
from employee;

-- 입사일 부터 1000일 지난 시점의 날짜를 출력
select ename 이름 , hiredate 입사일 , hiredate + 1000 입사1000일날짜
from employee;

-- 특정 날짜에서 월을 기준으로 출력
select hiredate 원본 , trunc(hiredate, 'MONTH') ,
ROUND (hiredate , 'MONTH')      --  15일이상부터는 반올림되어 출력
from employee;

-- 두 날짜사이의 개월수를 출력 : months_between
select ename 사원이름 , hiredate 입사일 , round(months_between (sysdate,hiredate))총개월수
from employee;

-- add_months (날짜,개월수) : 날짜에 개월수를 더하는 함수
select add_months (sysdate , 100) as "100개월후", sysdate + 100 as "100일후"
from dual;

-- 입사후 6개월이 지난 시점을 출력
select ename , hiredate , add_months(hiredate ,6) "입사후6개월시점"
from employee;

-- next_day : 특정날짜에서 요일을 인풋으로 넣어서 그 날짜가 도래하면 출력
select next_day (sysdate , '토요일')
from dual;

-- last_day(date) : 그 달의 마지막 날짜를 출력
